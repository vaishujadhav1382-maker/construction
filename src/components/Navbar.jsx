      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-luxury-beige/95 backdrop-blur-2xl z-40 flex flex-col items-center justify-center gap-8 lg:hidden"
          >
            {navLinks.map((link) => (
              <Link
                key={link.to}
                ...
                className="text-3xl font-outfit font-black uppercase tracking-tighter text-charcoal hover:text-luxury-gold"
              >
                {link.name}
              </Link>
            ))}
